...
import { IDeferred, Mapping } from '../types/interfaces';
import { log } from './log';

class QueueWorkerItem<T> {
    item: T;
    promises: Deferred<any>[];
}

export class QueueWorker<TIn, TOut> {
    private queue: Mapping<QueueWorkerItem<TIn>> = {};
    private current: Mapping<QueueWorkerItem<TIn>> = {};
    ...
    public async enqueue(item: TIn, id?: string): Promise<TOut> {
        let p = helpers.newPromise<TOut, any>();
        id = id || helpers.randomNumber().toString();
        let handler = this.queue[id];
        if (handler == null) handler = this.queue[id] = { item: item, promises: [] };
    ...
    private async cycle(): Promise<void> {
        ...
        this.current[id] = job;
        this.processor
            .apply(this.context, [job.item, id])
            .catch((ex) => {
                log.w('QueueWorker:cycle: Error processing item', job.item, ex);
                this.faultsCountDown--;
                if (this.faultsCountDown == 0) {
                    job.promises.forEach((x) =>
                        x.reject(new Error('QueueWorker:cycle: Could not recover from repeated failures! ex: ' + ex.response))
                    );
                    return;
                }
