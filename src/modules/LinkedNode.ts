...
import { JSONObject } from "../types/interfaces";
...
export default class LinkedNode<T> {
	...
	public removeChild(node: LinkedNode<T>): LinkedNode<T> {
		ArrayExtensions.remove.call(this.children, node);
		return this;
	}
	...
	public removeChildByContent(_content: T): LinkedNode<T> {
		...
		for(let child of this.children) {
			...
			ret = child;
			ArrayExtensions.remove.call(this.children, child);
			break;
		}
		return ret;
	}
	...
	public toString() {
		...
		if (ret == '[object Object]') ret = helpers.jsonify(content);
		return ret;
	}
