...
export class UserManager {
	...
	public signInGoogle = async () => {
		...
		if (this.auth.currentUser != null && this.auth.currentUser.isAnonymous)
			this.auth.currentUser.linkWithPopup(provider).then(()=>p.resolve()).catch(ex=>p.reject(ex));
		else
			this.auth.signInWithPopup(provider).then(()=>p.resolve()).catch(ex=>p.reject(ex));
		return p;
	};
	...
	public signInAnon = async (displayName) => {
		...
		this.auth.signInAnonymously().then(function (u) {
	...
	public signUpEmail = async (email, password) => {
		let p = helpers.newPromise();
		this.auth.createUserWithEmailAndPassword(email, password).then(()=>p.resolve()).catch(function(error) {
	...
	public signInEmail = async (email, password) => {
		...
		if (this.auth.currentUser != null && this.auth.currentUser.isAnonymous) {
			...
			this.auth.currentUser.linkWithCredential(credential).then(function(user) {
		...
		this.auth.signInWithEmailAndPassword(email, password).then(()=>p.resolve()).catch(function(error) {
