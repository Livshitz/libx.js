...
test('queue with maxConcurrent 1', async () => {
	...
	p1.then((res)=> { expect(new Date().getTime()-startDate).toBeLessThanOrEqual(delayPadded) })
	let p2 = queueWorker.enqueue("aa2", "2");
	p2.then((res)=> { expect(new Date().getTime()-startDate).toBeLessThanOrEqual(delayPadded*2) })
	let p3 = queueWorker.enqueue("aa3", "3");
	p3.then((res)=> { expect(new Date().getTime()-startDate).toBeLessThanOrEqual(delayPadded*3) })
...
test('queue with maxConcurrent 2', async () => {
	...
	let p2 = queueWorker.enqueue("aa2", "2");
	p1.then((res)=> { expect(new Date().getTime()-startDate).toBeLessThanOrEqual(delayPadded) })
	p2.then((res)=> { expect(new Date().getTime()-startDate).toBeLessThanOrEqual(delayPadded) })
	let p3 = queueWorker.enqueue("aa3", "3");
	p3.then((res)=> { expect(new Date().getTime()-startDate).toBeLessThanOrEqual(delayPadded*2) })
...
test('queue with maxConcurrent 3', async () => {
	...
	p1.then((res)=> { expect(new Date().getTime()-startDate).toBeLessThanOrEqual(delayPadded) })
	p2.then((res)=> { expect(new Date().getTime()-startDate).toBeLessThanOrEqual(delayPadded) })
	p3.then((res)=> { expect(new Date().getTime()-startDate).toBeLessThanOrEqual(delayPadded) })
